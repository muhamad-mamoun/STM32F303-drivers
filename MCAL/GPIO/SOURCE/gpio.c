...
void GPIO_init(void)
{
    ...
    for(uint8 LOC_portIterator = 0; LOC_portIterator < GPIO_NUM_OF_PORTS; LOC_portIterator++)
    {
        for(uint8 LOC_pinIterator = 0; LOC_pinIterator < GPIO_NUM_OF_PINS_PER_PORT; LOC_pinIterator++)
        {
 ...
 *                                                          - Port ID Error.
 ====================================================================================================================*/
GPIO_errorStatusType GPIO_readPort(GPIO_portIDType a_portID, uint16* a_ptr2portStatus)
{
    GPIO_errorStatusType LOC_errorStatus = GPIO_NO_ERRORS;
    GPIOx_registersType* LOC_ptr2GPIOx[GPIO_NUM_OF_PORTS] = {GPIOA,GPIOB,GPIOC,GPIOD,GPIOE,GPIOF,GPIOG,GPIOH};

    if(a_ptr2portStatus == NULL_PTR)
    {
        LOC_errorStatus = GPIO_NULL_PTR_ERROR;
    }
